...
def topic_model_new(predict,
  ...
  rec_vector_1 = Weight((n_concept, 500))(f_input)
  rec_vector_2 = Weight((500, f_train.shape[3]))(f_input)
  rec_vector_linear = Weight((n_concept, f_train.shape[3]))(f_input)
  ...
  ## option: test 1 linear layer
  #rec_layer_linear = Lambda(lambda x:(K.dot(x[0],x[1])))([topic_prob_nn, rec_vector_linear])
  #rec_layer_n =  Lambda(lambda x:K.l2_normalize(x,axis=(3)))(rec_layer)
  #pred = predict(rec_layer_linear)
  ...
  if load:
    ...
    #  need to tune parameters in topic loss for different data
    topic_model_pr.fit(
            f_train,
            y_train,
